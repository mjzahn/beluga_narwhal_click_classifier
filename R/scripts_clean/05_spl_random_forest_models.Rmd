...
```{r}
## see if we can plot the noise floor
calculateAverageSpectra(fish_dets_0kHzPP_filter,evNum='9',noise=TRUE,wl=256)
calculateAverageSpectra(fish_dets_0kHzPP_filter,evNum='9',noise=TRUE,wl=512)

calculateAverageSpectra(fish_dets_0kHzPP_filter,evNum='13',noise=TRUE,wl=128)
calculateAverageSpectra(fish_dets_0kHzPP_filter,evNum='13',noise=TRUE,wl=512)
```
...
fish_test_data_select <- fish_test_data %>% select(c(species,spl_diff_16to23_dB,spl_diff_23to46_dB)) %>% filter(species!='NOISE')
kong_test_data_select <- kong_test_data %>% select(c(species,spl_diff_16to23_dB,spl_diff_23to46_dB)) %>% filter(species!='NOISE')

SPL_diff_df <- rbind(fish_test_data_select,kong_test_data_select)
```
...
## Run random forest model (without peakTime, dBPP, and noiseLevel)
ranfor_soundtrap <- randomForest(species ~ spl_diff_16to23_dB + spl_diff_23to46_dB,
                       data = SPL_diff_df, 
                       ntree = 10000, 
                       importance = TRUE,
                       replace = TRUE, 
...
ranfor <- randomForest(species ~ spl_diff_16to23_dB + spl_diff_23to46_dB, 
                       data = train_spl_data, 
                       ntree = 10000, 
                       importance = TRUE,
                       replace = TRUE, 
...
y_pred <- predict(ranfor, newdata=SPL_diff_df[,2:3])
y_pred

## make confusion matrix
table(SPL_diff_df[,1], y_pred)
...
y_pred <- predict(ranfor_soundtrap, newdata=train_spl_data[,2:3])
y_pred
...
```{r run RF models with noise category}
fish_test_data <- read.csv(here("R/spreadsheets/SPL_difference/model_input/testing_fish_data_spl_diff_0kHz_highpass.csv"))
kong_test_data <- read.csv(here("R/spreadsheets/SPL_difference/model_input/testing_kong_data_spl_diff_0kHz_highpass.csv"))

## select columns we need
fish_test_data_select <- fish_test_data %>% select(c(species,spl_diff_16to23_dB,spl_diff_23to46_dB))
kong_test_data_select <- kong_test_data %>% select(c(species,spl_diff_16to23_dB,spl_diff_23to46_dB))
...
```{r run seperate models with noise}
## make sure species is a factor
fish_test_data_select$species <- as.factor(fish_test_data_select$species)
kong_test_data_select$species <- as.factor(kong_test_data_select$species)
